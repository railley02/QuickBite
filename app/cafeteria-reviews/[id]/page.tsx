...
import { getCafeteria } from "@/lib/fake-api"
...
function ReviewsContent() {
  ...
  useEffect(() => {
    async function loadData() {
      const caf = await getCafeteria(id)
      setCafeteria(caf)
    }
    loadData()
  }, [id])
  ...
  return (
    <main className="min-h-dvh bg-background flex flex-col">
      {/* Header */}
      <div className="bg-gradient-to-b from-primary to-orange-400 px-4 pt-6 pb-6">
        <button onClick={() => router.back()} className="text-white mb-4 cursor-pointer">
          <ArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="text-xl font-bold text-white">{cafeteria.name} Reviews</h1>
        <div className="flex items-center gap-2 mt-2">
          <Star className="w-5 h-5 text-white fill-white" />
          <span className="text-white font-semibold">{cafeteria.rating}</span>
          <span className="text-white/80">({cafeteria.totalRatings} reviews)</span>
        </div>
      </div>
...
export default function CafeteriaReviewsPage() {
  return (
    <Suspense
      fallback={
        <main className="min-h-dvh bg-background flex items-center justify-center">
