  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        customer_email: user?.email || "guest@quickbite.app",
        stall_id: cafeteriaId, // This is the stall UUID from the stalls table
        stall_name: cafeteriaName,
        items: items.map((i) => ({
          id: i.item.id,
          name: i.item.name,
          price: i.item.price,
      ...
      <div className="flex-1 px-4 py-6 pb-32 overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            ...
            {user && <p className="text-xs text-primary mb-2">Ordering as: {user.email}</p>}
            {items.map((cartItem) => (
