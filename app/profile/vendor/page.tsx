...
export default function VendorProfilePage() {
  ...
  const { isOpen, todaySales, vendorOrders } = useVendor()
  ...
  const { error } = await supabase
    .from("users")        // make sure this is the table where vendor info is stored
  ...
  const { error } = await supabase
    .from("vendors")
    .update({ operating_hours: editHours })
    .eq("user_id", userId)
  ...
  const { error } = await supabase
    .from("vendors")
    .update({ slot_capacity: editSlotCapacity })
    .eq("user_id", userId)


  if (error) {
    alert("Failed to update slot capacity: " + error.message)
  } else {
  ...
  useEffect(() => {
    const fetchVendorProfile = async () => {
      ...
      if (!user) {
        console.warn("User record not found yet")
        return
      }

      // 2️⃣ Fetch vendor profile
      const { data: vendor, error: vendorError } = await supabase
        .from("vendors")
        .select("stall_name, stall_number, operating_hours, slot_capacity")
        .eq("user_id", userId)
        .maybeSingle()
      ...
      if (!vendor) {
        console.warn("Vendor record not found yet")
        return
      }
      ...
      setProfile({
        ...
        email: user.email,
        phone: user.contact,
        stallName: vendor.stall_name,        // ✅ ADD THIS
        stallNumber: vendor.stall_number,
        operatingHours: vendor.operating_hours,
        slotCapacity: vendor.slot_capacity,
      })


      setEditPhone(user.contact)
      setEditHours(vendor.operating_hours)
      setEditSlotCapacity(vendor.slot_capacity)
    }
        ...
        <div className="bg-card border border-border rounded-xl p-4 shadow-md">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              {/* ✅ CHANGE THIS: Use stallName instead of name */}
              <h2 className="font-semibold text-foreground text-lg">
                {profile.stallName || "No Stall Name"}
              </h2>

              <p className="text-sm text-muted-foreground">
                {profile.stallNumber || "No Stall Number"}
              </p>
