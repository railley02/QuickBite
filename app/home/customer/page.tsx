...
// ✅ ADD: menuItems as mockMenuItems
import { 
  cafeterias as mockCafeterias, 
  menuItems as mockMenuItems // 👈 Add this
} from "@/lib/mock-data"
import { getCafeteriasFromDB } from "@/lib/cafeterias-api"
...
export default function CustomerHomePage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const dbCafeterias = await getCafeteriasFromDB()

      const mergedCafeterias = [
        ...mockCafeterias,
        ...dbCafeterias.filter(
          (db) => !mockCafeterias.some((mock) => mock.id === db.id)
        ),
      ]

      // ✅ FIX 1: Save the cafeterias to state! 
      // (This was missing before, which caused the empty list)
      setCafeterias(mergedCafeterias)

      // ✅ FIX 2: Use the imported items directly for recommendations
      // (This fixes the "property 'menu' does not exist" error)
      setRecommendedItems(mockMenuItems.slice(0, 6))
  ...
  // Live search (client-side, DB-backed)
  useEffect(() => {
    if (searchQuery.trim() === "") {
